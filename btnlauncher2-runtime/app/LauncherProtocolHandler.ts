 ...
 *
 * The drop in request or localhost request will be transformed into btnlauncher2:// protocol
 */
  ...
  async handle(request: RequestOptions): Promise<ResolvedResponse> {
    ...
    const context: Context = {
      request: {
        ...
        url: typeof request.url === 'string' ? new URL(request.url, 'btnlauncher2://launcher') : request.url,
