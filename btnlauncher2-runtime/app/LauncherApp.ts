import { getPlatform } from '@btnlauncher2/core'
import { AgentRunTrace, InstalledAppManifest, Platform, createPromiseSignal } from '@btnlauncher2/runtime-api'
...
export class LauncherApp extends EventEmitter {
  /**
   * Launcher %APPDATA%/btnlauncher2 path
   */
  ...
  get userAgent() {
    ...
    return `voxelum/x_minecraft_launcher/${version} (btnlauncher2.app)`
  }
  ...
  readonly server: Server = createServer((req, res) => {
    this.protocol.handle({
      method: req.method,
      url: new URL(req.url ?? '/', 'btnlauncher2://launcher'),
      headers: req.headers,
      body: req,
    }).then((resp) => {
  ...
  /**
   * Determine the root of the project. By default, it's %APPDATA%/btnlauncher2
   */
  protected async setup() {
    ...
    // register btnlauncher2 protocol
    if (!this.host.isDefaultProtocolClient('btnlauncher2')) {
      const result = this.host.setAsDefaultProtocolClient('btnlauncher2')
      if (result) {
        this.logger.log('Successfully register the btnlauncher2 protocol')
      } else {
        this.logger.log('Fail to register the btnlauncher2 protocol')
      }
    }
  ...
  protected async getStartupUrl(): Promise<string | undefined> {
    if (!IS_DEV && process.platform === 'win32') {
      ...
      if (process.argv.length > 1) {
        ...
        const protocolOption = process.argv.find(a => a.startsWith('btnlauncher2://'))
        if (protocolOption) {
        ...
        this.logger.log('Didn\'t find btnlauncher2:// protocol')
      }
    }
