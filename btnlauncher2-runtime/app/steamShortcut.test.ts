...
describe('addSteamShortcutToVdf', () => {
  const shortcut = {
    executable: '"/home/deck/Btn Minecraft Launcher"',
    startDir: '"/home/deck"',
    icon: '/home/deck/Btn Minecraft Launcher',
  }
  ...
  test('appends a shortcut without rewriting existing binary-VDF entries', () => {
    ...
    expect(updated!.toString('utf8')).toContain('Btn Minecraft Launcher')
  })

  test('does not add a second launcher shortcut', () => {
    const launcher = object('0', string('AppName', 'Btn Minecraft Launcher'))
