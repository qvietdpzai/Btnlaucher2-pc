import { AppManifest, AppsHost, InstalledAppManifest } from '@btnlauncher2/runtime-api'
...
export class LauncherAppManager implements AppsHost {
  ...
  async tryGetInstalledApp(url: string): Promise<InstalledAppManifest | undefined> {
    ...
    const validJson = await readFile(join(path, 'app.btnlauncher2x'), 'utf8').then(JSON.parse, (e) => {
  ...
  async getInstalledApp(url: string): Promise<InstalledAppManifest> {
    ...
    const path = this.getAppRoot(url)
    return readFile(join(path, 'app.btnlauncher2x'), 'utf8').then(JSON.parse)
  }

  async getInstalledApps(): Promise<InstalledAppManifest[]> {
    await ensureDir(this.root)
    const files = await readdir(this.root).then((files) => files.map(f => join(this.root, f, 'app.btnlauncher2x')))
  ...
  async installApp(url: string, options: InstallAppOptions = {}) {
    ...
    if (this.app.platform.os === 'windows') {
      ...
      await writeFile(join(appDir, 'app.btnlauncher2x'), JSON.stringify(appMan))
