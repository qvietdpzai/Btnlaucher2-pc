import { CURRENT_COLLECTION_SCHEMA_VERSION, LocalCollectionState } from '@btnlauncher2/runtime-api'
import { mkdtemp, readFile, rm, writeFile } from 'fs-extra'
import { tmpdir } from 'os'
import { join } from 'path'
import { afterEach, beforeEach, describe, expect, test, vi } from 'vitest'

// The `~/` alias points at btnlauncher2-runtime and is not configured for Vitest.
...
describe('CollectionService', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'btnlauncher2-collection-'))
  })
