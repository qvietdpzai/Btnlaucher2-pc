 ...
 *
 * Run with:
 *   BMCL_PROBE=1 npx vitest run btnlauncher2-runtime/network/BmclDownload.e2e.test.ts
 *
...
import type { DownloadResult } from '@btnlauncher2/file-transfer'
...
run('BMCLAPI adaptive download e2e', () => {
  it(
    ...
    async () => {
      ...
      const dir = await mkdtemp(join(tmpdir(), 'btnlauncher2-bmcl-'))
      try {
