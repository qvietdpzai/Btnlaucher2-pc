...
import { z } from 'zod'
import { defineCommand, CommandRegistry } from '@btnlauncher2/runtime-api'
...
describe('runCli', () => {
  test('returns handled=false when argv has no command', async () => {
    ...
    const r = await runCli(host, { argv: ['/bin/btnlauncher2'] })
  ...
  test('--help prints top-level help and exits 0', async () => {
    const host = makeHost(() => undefined)
    const r = await runCli(host, { argv: ['btnlauncher2', '--help'] })
  ...
  test('--version prints app.version and exits 0', async () => {
    const host = makeHost(() => undefined)
    const r = await runCli(host, { argv: ['btnlauncher2', '--version'] })
    expect(r.exitCode).toBe(0)
  ...
  test('command with -h prints command help', async () => {
    const host = makeHost(() => undefined)
    const r = await runCli(host, { argv: ['btnlauncher2', 'launch', '-h'] })
    expect(r).toMatchObject({ handled: true, exitCode: 0 })
  ...
  test('dispatches a parsed command to the host', async () => {
    ...
    const r = await runCli(host, { argv: ['btnlauncher2', 'launch', '/inst/foo', '--user', 'u1'] })
    expect(r.exitCode).toBe(0)
  ...
  test('enrichInput is applied before dispatch', async () => {
    ...
    const host = makeHost(handler)
    await runCli(host, {
      argv: ['btnlauncher2', 'launch', '/inst/foo'],
  ...
  test('schema validation error returns exit 2 and human error', async () => {
    const host = makeHost(() => undefined)
    const r = await runCli(host, { argv: ['btnlauncher2', 'launch', '/inst/foo', '--mod-count', 'NaN'] })
  ...
  test('handler throwing returns exit 1', async () => {
    ...
    const r = await runCli(host, { argv: ['btnlauncher2', 'launch', '/inst/foo'] })
  ...
  test('--json error envelope', async () => {
    const host = makeHost(() => { throw new Error('boom') })
    await runCli(host, { argv: ['btnlauncher2', '--json', 'launch', '/inst/foo'] })
  ...
  test('parse errors (e.g. unknown flag) return exit 2', async () => {
    const host = makeHost(() => undefined)
    const r = await runCli(host, { argv: ['btnlauncher2', 'launch', '/inst/foo', '-z', 'x'] })
    expect(r.exitCode).toBe(2)
  })
})
