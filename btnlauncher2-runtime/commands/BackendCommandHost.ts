...
import type { Instance } from '@btnlauncher2/instance'
import type { UserProfile } from '@btnlauncher2/runtime-api'
  ...
  createContext(opts: { mode?: CommandMode; signal?: AbortSignal; out?: CommandOutput } = {}): CommandContext {
    ...
    const ctx: CommandContext = {
      ...
      async resolveInstance(ref: string): Promise<Instance> {
        const { InstanceServiceKey } = await import('@btnlauncher2/runtime-api')
      ...
      async resolveUser(ref?: string): Promise<UserProfile> {
        const { UserServiceKey } = await import('@btnlauncher2/runtime-api')
