...
import { onDownloadSingle, Tracker } from '@btnlauncher2/installer'
import { isValidModrinthId, ResourceDomain, ResourceManager, type Resource } from '@btnlauncher2/resource'
import { kResourceManager } from '~/resource'
import { ModMetadataServiceKey, type ModMetadataService as IModMetadataService, type ModMetadata, DownloadModMetadataDbTask, DownloadModMetadataDbTrackerEvents } from '@btnlauncher2/runtime-api'
...
import { jsonObjectFrom, NodeSqliteDialect } from '@btnlauncher2/sqlite'
...
export class ModMetadataService extends AbstractService implements IModMetadataService {
  ...
  async #ensureDb() {
    ...
    const sha1 = await (await this.app.fetch('https://btnlauncher2.blob.core.windows.net/releases/db.sqlite.sha1')).text()
    const dbPath = this.getAppDataPath('db.sqlite')
    const actual = await checksumFromStream(createReadStream(dbPath), 'sha1').catch(() => '')
    if (actual !== sha1) {
      const url = 'https://btnlauncher2.blob.core.windows.net/releases/db.sqlite'
