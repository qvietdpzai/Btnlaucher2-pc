import { ProjectMappingService as IProjectMappingService, ProjectMapping, ProjectMappingServiceKey, Settings } from '@btnlauncher2/runtime-api'
...
import { NodeSqliteDialect } from '@btnlauncher2/sqlite'
...
export class ProjectMappingService extends AbstractService implements IProjectMappingService {
  ...
  private async ensureDatabaseFile(locale: string, forceDownload = false) {
    ...
    await this.mutex.of('project-mapping').runExclusive(async () => {
      let original = `https://btnlauncher2.blob.core.windows.net/project-mapping/${locale}.sqlite`
      ...
      if (!hasLocaleDb) {
        original = 'https://btnlauncher2.blob.core.windows.net/project-mapping/en.sqlite'
