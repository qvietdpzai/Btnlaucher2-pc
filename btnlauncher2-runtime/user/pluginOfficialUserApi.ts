...
import { MicrosoftAuthenticator, MojangClient } from '@btnlauncher2/user'
  ...
  const system = new MicrosoftAccountSystem(logger,
    ...
    new MicrosoftOAuthClient(
      ...
      async (directRedirectToLauncher) => {
        ...
        return (directRedirectToLauncher ? `http://localhost:${port}/auth` : `https://btnlauncher2.app/auth?port=${port}`)
      },
      (response) => {
