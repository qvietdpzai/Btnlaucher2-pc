...
export class MicrosoftOAuthClient {
  ...
  private prepareNativeBrokerRuntime() {
    ...
    const target = join(tmpdir(), 'btnlauncher2-msal-node-runtime', String(process.pid), arch)
    ...
    process.env.Btnlauncher2_MSAL_NODE_RUNTIME_PATH = targetNode
  }
  ...
  protected async getOAuthApp(account: string, signal?: AbortSignal, nativeBrokerPlugin?: INativeBrokerPlugin) {
    return new PublicClientApplication({
      ...
      cache: {
        cachePlugin: createPlugin('btnlauncher2-oauth', 'Btnlauncher2_MICROSOFT_ACCOUNT', this.logger, this.storage),
      },
      system: {
        loggerOptions: {
