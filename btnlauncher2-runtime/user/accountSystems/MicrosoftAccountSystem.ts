import { AUTHORITY_MICROSOFT, AuthorityMetadata, GameProfileAndTexture, LoginOptions, RefreshUserOptions, Skin, SkinPayload, UserException, UserProfile, normalizeUserId } from '@btnlauncher2/runtime-api'
import { MicrosoftAuthenticator, MicrosoftMinecraftProfile, MicrosoftMinecraftXboxLoginError, MojangClient, MojangError } from '@btnlauncher2/user'
...
export class MicrosoftAccountSystem implements UserAccountSystem {
  ...
  protected async loginMicrosoft(microsoftEmailAddress: string, oauthCode: string | undefined, useDeviceCode: boolean, directRedirectToLauncher: boolean, signal: AbortSignal, slientOnly = false) {
    ...
    const { result, extra } = await this.oauthClient.authenticate(microsoftEmailAddress, MICROSOFT_XBOX_LAUNCHER_SCOPES, {
      code: oauthCode,
      // btnlauncher2 identity binding verifies the stable subject through Microsoft
