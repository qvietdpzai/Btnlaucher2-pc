import { CurseforgeV1Client } from '@btnlauncher2/curseforge'
import {
  InstanceLockSchema,
...
import { ModrinthV2Client } from '@btnlauncher2/modrinth'
import { isValidCurseforgeRef, isValidModrinthRef, ResourceManager, ResourceMetadata, UpdateResourcePayload } from '@btnlauncher2/resource'
import {
  ModpackException,
  ModpackServiceKey,
  ModpackState,
...
import { readEntry } from '@btnlauncher2/unzip'
import { AnyError } from '@btnlauncher2/utils'
import { LibraryInfo } from '@btnlauncher2/core'
import filenamify from 'filenamify'
import { ensureFile, readJson, stat, unlink, writeFile } from 'fs-extra'
import { basename, dirname, join, relative } from 'path'
import { Entry, ZipFile as YauzlZipFile } from '@btnlauncher2/yauzl'
...
type SelectedBtnlauncher2Fields = Pick<
  InstanceData,
  | 'disableElybyAuthlib'
...
export class ModpackService extends AbstractService implements IModpackService {
  ...
  async importModpack(
    ...
    // the merged result into a standalone version json and pin it, so btnlauncher2 does
  ...
  async exportModpack(options: ExportModpackOptions) {
    ...
    let btnlauncher2ManifestExtension: SelectedBtnlauncher2Fields = {
    ...
    const btnlauncher2Json = Buffer.from(JSON.stringify(btnlauncher2ManifestExtension, null, 4))
    curseforgeZip?.addBuffer(btnlauncher2Json, 'btnlauncher2.json')
    modrinthZip?.addBuffer(btnlauncher2Json, 'btnlauncher2.json')
    offlineZip?.addBuffer(btnlauncher2Json, 'btnlauncher2.json')
  ...
  async openModpack(modpackFile: string): Promise<SharedState<ModpackState>> {
    ...
    return store.registerOrGet(`modpack-file://${modpackFile}`, async () => {
      ...
      let btnlauncher2Cache: SelectedBtnlauncher2Fields | undefined
      if (zip.entries['btnlauncher2.json']) {
        try {
          const b = await readEntry(zip.file, zip.entries['btnlauncher2.json'])
          btnlauncher2Cache = JSON.parse(b.toString()) as SelectedBtnlauncher2Fields
        } catch {}
      }
