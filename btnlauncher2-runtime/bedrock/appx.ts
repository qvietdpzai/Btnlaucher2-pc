...
export async function registerPackage(packageFamily: string, gameDir: string): Promise<void> {
  ...
  const script =
    ...
    '  Write-Output \'Btnlauncher2_REGISTER_OK\';' +
    '} catch {' +
    '  Write-Output ("Btnlauncher2_REGISTER_ERR:" + $_.Exception.Message);' +
    '}'
  const out = (await runPowerShell(script)).trim()
  if (out.includes('Btnlauncher2_REGISTER_OK')) {
    return
  }
  ...
  throw new Error(out.replace(/^Btnlauncher2_REGISTER_ERR:/, '') || 'Failed to register the Bedrock package.')
}
