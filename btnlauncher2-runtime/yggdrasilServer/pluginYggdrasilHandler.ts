import { AUTHORITY_DEV } from '@btnlauncher2/runtime-api'
import { YggdrasilTexture, YggdrasilTexturesInfo } from '@btnlauncher2/user'
...
export const pluginYggdrasilHandler: LauncherAppPlugin = (app) => {
  ...
  app.protocol.registerHandler('btnlauncher2', async ({ request, response, handle }) => {
    if (request.url.pathname.startsWith('/yggdrasil')) {
      ...
      if (pathname === '/' || pathname === '') {
        ...
        response.body = JSON.stringify({
          meta: {
            implementationName: 'btnlauncher2-offline-server',
            implementationVersion: '0.0.1',
            serverName: 'Btn Minecraft Launcher Offline Server',
          },
          skinDomains: [
            'localhost',
          ],
