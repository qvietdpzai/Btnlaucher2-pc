...
export const pluginFlights: LauncherAppPlugin = async (app) => {
  ...
  const fetchFlights = async (output: Record<string, any>, cachedPath: string) => {
    try {
      ...
      const primary = await app.fetch(`https://api.btnlauncher2.app/flights?${queryString}`).catch(() => undefined)
      ...
      const resp = primary?.ok
        ? primary
        : await app.fetch(`https://btnlauncher2-core-api.azurewebsites.net/api/flights?${queryString}`)
      if (resp.status !== 200) {
