...
import { Btnlauncher2AccountApi, Btnlauncher2AccountApiError } from './Btnlauncher2AccountApi'
import { M1_LOCAL_AUTH_FIXTURE, M1_SHARED_V1_POLICY_FIXTURE } from './fixtures'

describe('Btnlauncher2AccountApi', () => {
  it('uses the injected btnlauncher2 API origin', async () => {
    const fetch = vi.fn(async (_input: string | URL, _init?: RequestInit) =>
      Response.json(M1_LOCAL_AUTH_FIXTURE),
    )
    const api = new Btnlauncher2AccountApi(fetch, 'https://edge.example.test/')
  ...
  it('keeps provider and btnlauncher2 credentials in main-process requests', async () => {
    const fetch = vi.fn(async (_input: string | URL, _init?: RequestInit) =>
      Response.json(M1_LOCAL_AUTH_FIXTURE),
    )
    const api = new Btnlauncher2AccountApi(fetch)
  ...
  it('creates a one-time browser authorization before opening the provider', async () => {
    ...
    const authorization = await api.beginBrowserAuthorization('google', {
      state: 'state-1',
      redirectUri: 'http://127.0.0.1:1234/btnlauncher2-auth',
    ...
    expect(url.searchParams.get('redirectUri')).toBe('http://127.0.0.1:1234/btnlauncher2-auth')
  ...
  it('rejects M6 accounting fields on the D1/D4 policy endpoint', async () => {
    const api = new Btnlauncher2AccountApi(async () =>
      Response.json({
  ...
  it('reports identity conflicts without exposing response details', async () => {
    ...
    expect(error).toBeInstanceOf(Btnlauncher2AccountApiError)
