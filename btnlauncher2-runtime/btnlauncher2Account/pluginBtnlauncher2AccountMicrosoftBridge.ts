...
export const pluginBtnlauncher2AccountMicrosoftBridge: LauncherAppPlugin = (app) => {
  const logger = app.getLogger('Btnlauncher2MicrosoftBridge')
    ...
    const attempt = app.registry
      ...
      .then((result) => {
        ...
        if (result === 'pending-consent') {
          logger.log('Btnlauncher2 btnlauncher2 bridge is waiting for Microsoft Graph consent.')
        }
      })
      .catch((e) => {
        logger.warn(
  ...
  void app.registry
    ...
    .catch(() => {
      logger.warn('Unable to load Microsoft users for btnlauncher2 Btnlauncher2 account bootstrap.')
    })
}
