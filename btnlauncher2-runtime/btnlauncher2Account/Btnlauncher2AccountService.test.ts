...
import { AUTHORITY_MICROSOFT } from '@btnlauncher2/runtime-api'
...
const { Btnlauncher2AccountService } = await import('./Btnlauncher2AccountService')

function createBtnlauncher2Service(stubBootstrapCredential = true) {
  ...
  const service = new Btnlauncher2AccountService(app as any, {} as any, store as any)
...
describe('Btnlauncher2AccountService Microsoft bootstrap', () => {
  ...
  it('uses the Windows native broker and native window handle for silent authentication', async () => {
    ...
    const { app, nativeWindowHandle, service, user } = createBtnlauncher2Service()
  ...
  it('uses the normal silent cache path without the native broker off Windows', async () => {
    ...
    oauth.authenticate.mockResolvedValue({ result: { accessToken: 'microsoft-token' } })
    const { service, user } = createBtnlauncher2Service()
  ...
  it('returns pending consent instead of throwing when Graph scope is unavailable silently', async () => {
    ...
    const { app, logger, service, user } = createBtnlauncher2Service()
  ...
  it('persists the rotated credential when the subsequent account snapshot fails', async () => {
    const { app, logger, service } = createBtnlauncher2Service()
    const previous = {
      sessionId: 'session-1',
      accountId: 'account-1',
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'Btnlauncher2 session refreshed; account snapshot refresh will retry later.',
    )
  })
})

describe('Btnlauncher2AccountService provider bootstrap queue', () => {
  it('serializes concurrent exchanges and links the second provider with the first session', async () => {
    const { app, service } = createBtnlauncher2Service(false)
  ...
  it('continues queued provider exchanges after a failed exchange', async () => {
    const { service } = createBtnlauncher2Service(false)
  ...
  it('does not send an expired btnlauncher2 credential during provider re-bootstrap', async () => {
    const { service } = createBtnlauncher2Service(false)
    const expired = {
