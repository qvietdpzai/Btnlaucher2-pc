import { MinecraftFolder, type LaunchOption as ResolvedLaunchOptions, type ResolvedVersion, type ServerOptions, type ResolvedServerVersion, createMinecraftProcessWatcher, generateArguments, generateArgumentsServer, launch, launchServer } from '@btnlauncher2/core'
import { AUTHORITY_DEV, type CreateLaunchShortcutOptions, type GameProcess, type LaunchService as ILaunchService, LAUNCH_FAILURE_PREFIX, LaunchException, type LaunchOptions, LaunchServiceKey, type ReportOperationPayload } from '@btnlauncher2/runtime-api'
...
import { ensureDir } from '@btnlauncher2/installer/utils'
...
export class LaunchService extends AbstractService implements ILaunchService {
  ...
  async #generateOptions(options: LaunchOptions, version: ResolvedVersion, accessToken?: string) {
    ...
    const launcherName = `Btn Minecraft Launcher (${this.app.version})`
  ...
  async launch(options: LaunchOptions) {
    // E2E hook: when BTNLAUNCHER2_E2E_NO_LAUNCH is set, never spawn a real Java
    ...
    if (process.env.BTNLAUNCHER2_E2E_NO_LAUNCH) {
      const fakePid = Math.floor(Math.random() * 100_000) + 100_000
      const operationId = options.operationId || randomUUID()
      this.log(`[E2E] BTNLAUNCHER2_E2E_NO_LAUNCH set; skipping real Java spawn (pid=${fakePid}).`)
   ...
   * written into the instance's `logs/` folder using a `btnlauncher2-abnormal-exit-`
  ...
  async #persistAbnormalExitLog(gameDirectory: string, info: {
    ...
    const header = [
      `# Btn Minecraft Launcher abnormal-exit dump`,
