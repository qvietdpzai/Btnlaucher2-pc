...
import { kBtnlauncher2SessionAuthorization, Btnlauncher2AccountService } from '~/btnlauncher2Account/Btnlauncher2AccountService'
...
export class PeerService extends StatefulService<PeerState> implements IPeerService {
  constructor(
    ...
    const getMultiplayerApiBaseUrl = async () => {
      ...
      return resolveBtnlauncher2ApiBaseUrl(flights.btnlauncher2ApiBaseUrl, app.getLogger('MultiplayerApi'))
    }
    const requestRoom = async (
      path: string,
      init: RequestInit,
    ): Promise<MultiplayerRoomAdmission | undefined> => {
      const account = await app.registry.get(Btnlauncher2AccountService)
      const authorization = await account[kBtnlauncher2SessionAuthorization]()
      if (!authorization) throw new Error('btnlauncher2_account_session_missing')
