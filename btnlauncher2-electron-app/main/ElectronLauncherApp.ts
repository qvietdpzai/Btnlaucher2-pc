import { NetworkErrorCode, NetworkException } from '@btnlauncher2/runtime-api'
import { LauncherApp, Shell } from '@btnlauncher2/runtime/app'
import { LAUNCHER_NAME } from '@btnlauncher2/runtime/constant'
import { kFlights } from '@btnlauncher2/runtime/infra'
...
export default class ElectronLauncherApp extends LauncherApp {
  ...
  constructor() {
    // E2E hook: when BTNLAUNCHER2_E2E_APP_DATA is set, redirect Electron's appData
    // (and therefore both `app.getPath('appData')` calls below + the one in
    // btnlauncher2-runtime/app/LauncherApp.ts) to a per-test temporary directory.
    // This isolates every Playwright spec on a clean launcher root.
    if (process.env.BTNLAUNCHER2_E2E_APP_DATA) {
      try {
        app.setPath('appData', process.env.BTNLAUNCHER2_E2E_APP_DATA)
        app.setPath('userData', join(process.env.BTNLAUNCHER2_E2E_APP_DATA, LAUNCHER_NAME))
      } catch {
    ...
    if (!process.env.BTNLAUNCHER2_E2E &&
  ...
  protected async setup() {
    ...
    app.on('open-url', (event, url) => {
      ...
      if (last.startsWith('btnlauncher2://')) {
