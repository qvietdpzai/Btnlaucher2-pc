...
import { Tracker, onDownloadSingle } from '@btnlauncher2/installer'
import {
  DownloadUpdateTrackerEvents,
  ElectronUpdateOperation,
  ReleaseInfo,
} from '@btnlauncher2/runtime-api'
import { DownloadUpdateOptions, LauncherAppUpdater } from '@btnlauncher2/runtime/app'
import { AnyError, isSystemError } from '@btnlauncher2/utils'
...
async function downloadAsarUpdate(
  ...
  const github = `https://github.com/qvietdpzai/Btnlaucher2-pc/releases/download/v${version}/${file}`
  ...
  // per-platform `@btnlauncher2/app-<platform>` package. npmmirror's per-file
  ...
  if (gfw.inside) {
    const tarball = `https://registry.npmmirror.com/@btnlauncher2/app-${platformFlag}/-/app-${platformFlag}-${version}.tgz`
    try {
...
async function hintUserDownload(): Promise<void> {
  shell.openExternal('https://btnlauncher2.app')
}
...
async function downloadAppInstaller(
  ...
  const destination = join(app.getPath('downloads'), 'Btn Minecraft Launcher.appinstaller')
  const url = 'https://btnlauncher2.blob.core.windows.net/releases/btnlauncher2.appinstaller'
...
export class ElectronUpdater implements LauncherAppUpdater {
  ...
  async #getUpdateFromSelfHost(): Promise<ReleaseInfo> {
    ...
    const primary = await this.app
      .fetch(`https://api.btnlauncher2.app/latest?${queryString}`, {
        headers: {
    ...
    const response = primary?.ok
      ? primary
      : await this.app.fetch(`https://btnlauncher2-core-api.azurewebsites.net/api/latest?${queryString}`, {
        headers: {
          'Accept-Language': locale,
        },
      })
