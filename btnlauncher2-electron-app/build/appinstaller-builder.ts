...
function getAppInstallerContent(version: string, publisher: string) {
  ...
  <AppInstaller
      ...
      Version="${version}.0"
      Uri="https://btnlauncher2.blob.core.windows.net/releases/btnlauncher2.appinstaller" >
      <MainPackage
          Name="Btnlauncher2"
          Publisher="${publisher}"
          Version="${version}.${process.env.BUILD_NUMBER || '0'}"
          ProcessorArchitecture="x64"
          Uri="https://btnlauncher2-core-api.azurewebsites.net/api/appx?version=${version}" />
      <UpdateSettings>
      </UpdateSettings>
  </AppInstaller>`
