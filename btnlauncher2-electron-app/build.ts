...
async function buildMain(options: BuildOptions, slient = false, copyMsalRuntime = process.platform === 'win32') {
  ...
  await copy(path.join(__dirname, '../btnlauncher2-keystone-ui/dist'), path.join(__dirname, './dist/renderer'))
...
async function start() {
  ...
  const config: Configuration = {
    ...
    async afterPack(context) {
      ...
      // pipeline reads this to tag the `@btnlauncher2/app` npm package, so the
    ...
    async artifactBuildCompleted(context) {
      ...
      if (context.target && context.target.name === 'appx') {
        await buildAppInstaller(version, path.join(__dirname, './build/output/btnlauncher2.appinstaller'), electronBuilderConfig.appx!.publisher!)
      }
    },
  }
