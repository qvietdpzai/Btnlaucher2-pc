...
describe('parseCli — globals + dispatch', () => {
  ...
  test('--root with value', () => {
    const r = parseCli(['--root', '/data/btnlauncher2', 'launch', '/inst/foo'], makeRegistry())
    expect(r.globals.root).toBe('/data/btnlauncher2')
  })
  ...
  test('skips electron-noise leading args until a command name is recognised', () => {
    const r = parseCli(['/usr/bin/btnlauncher2', '--gpu-blacklist=', 'launch', '/inst/foo'], makeRegistry())
...
describe('formatHelp / formatCommandHelp', () => {
  test('top-level help mentions command', () => {
    const txt = formatHelp(makeRegistry(), { programName: 'btnlauncher2' })
    expect(txt).toContain('btnlauncher2')
  ...
  test('command help shows positionals + flags', () => {
    ...
    const txt = formatCommandHelp(cmd, { programName: 'btnlauncher2' })
