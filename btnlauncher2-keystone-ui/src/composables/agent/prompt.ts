...
function capabilitySection(profile: AgentPromptProfile) {
  ...
  if (names.has('bash')) {
    lines.push('The `bash` tool accepts exactly one btnlauncher2 command as its input. Do not prefix the input with `bash` and do not use pipes, redirects, `&&`, or `||`.')
...
const launcherIdentity = `You are the primary btnlauncher2 (Btn Minecraft Launcher) assistant. Help the user manage the explicitly selected Minecraft instance, resources, Java, local servers, worlds, launch failures, and launcher settings.`
const cssIdentity = `You are the btnlauncher2 Custom CSS assistant. Your only responsibility is inspecting the launcher UI and maintaining the global custom CSS document. Do not manage Minecraft instances, resources, accounts, or game launch.`
