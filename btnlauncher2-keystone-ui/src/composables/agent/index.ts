import { AgentServiceKey, type AgentMessage, type AgentRunEvent } from '@btnlauncher2/runtime-api'
...
export function installAgentDevLauncher(session: AgentSession, enabled: Ref<boolean>) {
  ...
  watch(enabled, (on) => {
    if (on) (window as any).__btnlauncher2_agent = surface
    else delete (window as any).__btnlauncher2_agent
  }, { immediate: true })
