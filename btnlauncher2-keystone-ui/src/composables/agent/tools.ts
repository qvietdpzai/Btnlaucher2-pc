...
import type { EditInstanceOptions } from '@btnlauncher2/instance'
import type { Resource } from '@btnlauncher2/resource'
import {
  formatCommandHelp,
...
export function useAgentToolFactory() {
  ...
  async function createBashTool(context: AgentRunContext): Promise<AgentTool> {
    ...
    return {
      name: 'bash',
      label: 'BTNLAUNCHER2 command',
      description: 'Run one restricted btnlauncher2 renderer command.',
  ...
  function createUiTool(): AgentTool {
    return {
      name: 'ui',
      label: 'Launcher UI',
      description: 'Navigate or inspect the btnlauncher2 user interface. The run instance and user cannot be changed.',
...
export function useCssAgentToolFactory() {
  ...
  const uiTool: AgentTool = {
    name: 'ui',
    label: 'Launcher UI',
    description: 'Inspect the btnlauncher2 user interface for CSS selectors and computed styles.',
    parameters: actionObjectSchema(
