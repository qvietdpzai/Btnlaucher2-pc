...
import { BaseServiceKey, AUTHORITY_MICROSOFT, LaunchServiceKey } from '@btnlauncher2/runtime-api'
...
export function useFriendsPresence() {
  ...
  function setupMyPresenceSocket() {
    ...
    const url = `wss://api.btnlauncher2.app/group/presence-${myUUID}?client=${clientToken.value}`
  ...
  function updateFriendSockets() {
    ...
    for (const friend of currentFriends) {
      ...
      if (!friendSockets.has(friendId)) {
        const url = `wss://api.btnlauncher2.app/group/presence-${friendId}?client=${clientToken.value}`
        const ws = new WebSocket(url)
