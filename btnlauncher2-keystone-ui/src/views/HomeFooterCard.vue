...
import { InstanceModsServiceKey, InstanceResourcePacksServiceKey, InstanceSavesServiceKey, InstanceShaderPacksServiceKey, ModpackServiceKey } from '@btnlauncher2/runtime-api'
...
async function getVersionDetail(version: StoreProjectVersion): Promise<StoreProjectVersionDetail> {
  if (upstream.value?.type === 'modrinth-modpack') {
    ...
    try {
      ...
      const { FileRelationType } = await import('@btnlauncher2/curseforge')
