...
import { BaseServiceKey } from '@btnlauncher2/runtime-api'
...
/**
 * Exports all settings from localStorage into a downloadable .btnlauncher2settings file.
 */
async function handleExportSettings() {
  try {
    ...
    a.href = url
    a.download = `btnlauncher2-settings-${new Date().toISOString().split('T')[0]}.btnlauncher2settings`
...
/**
 * Prompts the user to select a .btnlauncher2settings file and imports the settings from it.
 */
async function handleImportSettings() {
  try {
    const input = document.createElement('input')
    input.type = 'file'
    input.accept = '.btnlauncher2settings'
