...
若是mod造成的崩溃，你应当通过日志分析出哪些mod可能造成这些错误，并提示我。注意我正在是用Btn Minecraft Launcher，关于修复崩溃的建议（如下载 Mod）应当在 Btn Minecraft Launcher 里进行。
...
For Java issues or missing vanilla game libraries, you should provide direct reminders. Please note that I am using Btn Minecraft Launcher, and any suggestions for fixing crashes (such as downloading mods) should be done within Btn Minecraft Launcher.
...
export function getCrashAgentPrompt(
  ...
  return `My Minecraft game crashed or failed to launch. Find the root cause and fix it for me within Btnlauncher2.
